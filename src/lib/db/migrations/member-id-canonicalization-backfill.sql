...
-- JOIN on members.clerk_id matches the provider-string columns.
...
WHERE i.member_id IS NULL
  AND i.invitee_user_id = m.clerk_id;
...
DO $$ BEGIN
  IF to_regclass('public.marketplace_submissions') IS NOT NULL THEN
    ...
    WHERE ms.submitter_member_id IS NULL
      ...
      AND ms.submitter_user_id = m.clerk_id;
  END IF;
END $$;
