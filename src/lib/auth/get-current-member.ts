...
export async function getCurrentMember(db: any): Promise<Member | null> {
  ...
  const logtoUser = nextAuthSession?.user as
  ...
  if (logtoUserId) {
    try {
      ...
      // Valid Castalia session but no member row yet. Provision on read
      // (mirrors the Clerk defensive-sync below) and re-query, so a fresh
      // Castalia user maps to a member on their first request instead of
      // 401-ing every protected route.
